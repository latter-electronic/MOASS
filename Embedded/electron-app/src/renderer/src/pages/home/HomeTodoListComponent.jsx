...
export default function HomeTodoListComponent() {
    ...
    return (
        <div className="bg-white/5 p-6 rounded-lg w-72 mt-5 h-60">
            <h1 className="text-white text-2xl font-bold mb-2">To-Do List ✨</h1>
            <div className="h-4/5 overflow-y-auto scrollbar-hide">
                <ul className="space-y-2">
                    {todos.map((todo) => (
                        <li key={todo.todoId} className="flex items-start text-xl">
                            <input
                                id={`todo-${todo.todoId}`}
                                type="checkbox"
