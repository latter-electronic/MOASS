...
def find_position(distances, beacon_coords):
    def objective(p):
        x, y = p
        return sum(((x - bx)**2 + (y - by)**2 - d**2)**2 for (bx, by), d in zip(beacon_coords, distances))

    x_guess = np.mean([coord[0] for coord in beacon_coords])
    y_guess = np.mean([coord[1] for coord in beacon_coords])
...
# 비콘 좌표 (예시로 6개 설정)
beacon_coords = [
    (0, 0),      # MOASS_1: 강의실 좌상단 모서리
    (942, 0),    # MOASS_2: 강의실 우상단 모서리
    (0, 1495),   # MOASS_3: 강의실 좌하단 모서리
    (942, 1495), # MOASS_4: 강의실 우하단 모서리
    (471, 0),    # MOASS_5: 강의실 중앙 상단
    (471, 1495)  # MOASS_6: 강의실 중앙 하단
]
...
while True:
    ...
    if len(distances) >= 3:
        available_beacons = [(beacon_coords[i], distances[beacon_id]) for i, beacon_id in enumerate(beacon_addresses.keys()) if beacon_id in distances]

        if len(available_beacons) >= 3:
            selected_coords, selected_distances = zip(*available_beacons)

            x, y = find_position(selected_distances, selected_coords)
            ...
            print('Not enough beacons for position calculation')
    else:
